 ...
 * @param cb
 */
rhymeApp.lift = (opt, cb) => {
    if (typeof opt === 'function') {
        cb = opt
        opt = {}
    }
    //把config,util注入this上下文
    app.use(function*(next) {
        this.config = merge(this.config, global[globalName].config)
        this.util = merge(this.util, global[globalName].util)
        yield next
    })
    boot(app, opt, cb)
    app.listen(global[globalName].config.port || 2333)
}


//把数组变成对象如:['a','b','c','d']=>{a:{b:{c:{d:{}}}}}
//当遇到index时,index下的属性会提升到和index平级的属x`性
...
function liftDatabase() {
    ...
    if (!rhyme.model || rhyme.config.useorm === false) return
...
//代理on
rhymeApp.on = function () {
    var args = Array.prototype.slice.call(arguments, 0)
    app.on.apply(app, args)
}
